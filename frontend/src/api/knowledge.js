...
export const getCitationGraph = async (doi, maxNodes = 60) => {
  const res = await apiClient.get(
    `/api/knowledge/citation-graph/${encodeURIComponent(doi)}?max_nodes=${maxNodes}`
  );
  return res.data;
};
...
export const getAuthorNetwork = async (authorId, limit = 50) => {
  const res = await apiClient.get(`/api/knowledge/author-network/${authorId}?limit=${limit}`);
  return res.data;
};

// Topic evolution by year
export const getTopicEvolution = async (keyword, years = 10) => {
  const res = await apiClient.get(`/api/knowledge/topic-evolution?keyword=${encodeURIComponent(keyword)}&years=${years}`);
  return res.data;
};
